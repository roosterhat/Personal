...
public class CommandInterpreter {
    ...
    public String interpretCommand(String arg){
        ArrayList<String> args = new ArrayList(Arrays.asList(arg.split(delimeter)));
        if(args.isEmpty())
        ...
        for(Command c : commands)
            if(c.equals(args.get(0))){
                if(c.checkArguments(args.size()-1))
                    ...
                    return "Invalid Syntax: expected "+c.displayPosibleArguments()+" argument"+(c.potentialArguments.size()==1?"":"s")+", found "+(args.size()-1)+" for command '"+c.name+"'";
            }                
