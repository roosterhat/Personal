...
// Camera intrinsics — CALIBRATE THESE for your actual lens/sensor.
// focal_px is approximate for a typical OV2640 at QQVGA; replace with a
// real calibration (e.g. checkerboard calibration scaled to this resolution)
// for anything beyond rough prototyping.
static float FOCAL_PX = 110.0f;
...
public:
  ...
  RotationEstimate update() {
    ...
    if (dt <= 0.0f) dt = 1.0f / 30.0f; // guard against timer wraparound edge case
...
private:
  ...
  static inline uint8_t px(const uint8_t *img, int x, int y) {
    if (x < 0) x = 0; if (x >= FRAME_W) x = FRAME_W - 1;
    if (y < 0) y = 0; if (y >= FRAME_H) y = FRAME_H - 1;
  ...
  static inline int gradMag(const uint8_t *img, int x, int y) {
    int gx = (px(img, x+1, y-1) + 2*px(img, x+1, y) + px(img, x+1, y+1))
            - (px(img, x-1, y-1) + 2*px(img, x-1, y) + px(img, x-1, y+1));
    int gy = (px(img, x-1, y+1) + 2*px(img, x, y+1) + px(img, x+1, y+1))
  ...
  bool refineLK(const uint8_t *prevImg, const uint8_t *currImg,
    ...
    for (int iter = 0; iter < LK_ITERATIONS; iter++) {
      ...
      if (fabs(du) < 0.05 && fabs(dv) < 0.05) break; // converged
    }
  ...
  void trackFeatures() {
    for (int i = 0; i < numFeatures; i++) {
      ...
      if (seedX < LK_WINDOW) seedX = LK_WINDOW;
      if (seedX > FRAME_W - 1 - LK_WINDOW) seedX = FRAME_W - 1 - LK_WINDOW;
      if (seedY < LK_WINDOW) seedY = LK_WINDOW;
      if (seedY > FRAME_H - 1 - LK_WINDOW) seedY = FRAME_H - 1 - LK_WINDOW;
  ...
  RotationEstimate fitRotation(float dt) {
    ...
    for (int i = 0; i < numFeatures; i++) {
      if (!features[i].track_ok) continue;
    ...
    if (keepCount < MIN_POINTS) keepCount = (n < MIN_POINTS) ? n : MIN_POINTS;
    ...
    for (int i = 0; i < keepCount; i++) {
      int k = order[i];
      X2[i] = X[k]; Y2[i] = Y[k]; U2[i] = U[k]; V2[i] = V[k];
    }
    ...
    float rmsSum = 0;
    for (int i = 0; i < keepCount; i++) rmsSum += residuals2[i] * residuals2[i];
  ...
  bool solveLeastSquares(const float *X, const float *Y, const float *U, const float *V,
    ...
    if (fabs(det) < 1e-9) return false; // degenerate (e.g. all points collinear)
