...
LinkedListIType LinkedList::begin(){
    LinkedList* temp = this;
    for(;temp->hasPrev();temp = temp->prev);
    return LinkedListIType(temp);
}

LinkedListIType LinkedList::end(){
    return LinkedListIType(nullptr);
}
...
void LinkedList::add(Value * value){    //adds a new value to the linked list in sorted order according to comparator, if no comparator exists diverts to append
...
void LinkedList::deleteSelf(){  //deletes current node from list
    if(hasPrev()){                           //if current node has previous node
        ...
        if(hasNext()){                      //if has next
            ...
            this->next = next->next;        //replace current's next node with the next node of current's next node
        }
        else
